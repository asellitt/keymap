 ...
 *  another key has been hit under the tapping term. This is typically
 *. indicitive that you are trying to "tap" the key.
 *
 ...
 *    choice for a tap dance.
 *  Letters used in common words as a double. For example 'p' in 'pepper'. If a
 *    tap dance function existed on the letter 'p', the word 'pepper' would be
 *    quite frustating to type.
 *
...
int current_dance(qk_tap_dance_state_t *state) {
  ...
  if (state->count == 1) {
    if (state->interrupted || !state->pressed) {
      ...
      // key has not been interrupted, but they key is still held. Means you
      //  want to send a 'HOLD'.
     ...
     *  this return value is when you want to send two keystrokes of the key,
     *  and not the 'double tap' action/macro.
     */
